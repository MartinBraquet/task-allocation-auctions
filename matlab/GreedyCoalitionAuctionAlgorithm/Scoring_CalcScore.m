...
%---------------------------------------------------------------------%
function [score minStart maxStart] = Scoring_CalcScore(GCAA_Params,agent,taskCurr,taskPrev,timePrev,taskNext,timeNext)
    
if((agent.type == GCAA_Params.AGENT_TYPES.QUAD) || ...
   (agent.type == GCAA_Params.AGENT_TYPES.CAR))
    ...
    % real score of GCAA paths once GCAA algorithm has finished
    % running.
...
% elseif(agent.type == GCAA_Params.AGENT_TYPES.NEW_AGENT), ...  

...
function fig = PlotAssignments(WORLD, GCAA_Assignments, agents, tasks, figureID)
...
for n=1:length(agents)
    ...
    if( GCAA_Assignments(n).path(1) > -1 )
        taskPrev = lookupTask(tasks, GCAA_Assignments(n).path(1));
        X = [agents(n).x, taskPrev.x];
        Y = [agents(n).y, taskPrev.y];
        T = [0, GCAA_Assignments(n).times(1)];
        ...
        agents(n).time = round(GCAA_Assignments(n).times(1)/100);
        agents(n).path = [taskPrev.x, taskPrev.y, agents(n).z, taskPrev.duration/100, taskPrev.id];
        
        for m = 2:length(GCAA_Assignments(n).path);
            if( GCAA_Assignments(n).path(m) > -1 )
                taskNext = lookupTask(tasks, GCAA_Assignments(n).path(m));
...
for n=1:length(agents),
    ...
    grid on;
    axis([0 1600 0 2])
    for m = 1:length(GCAA_Assignments(n).path),
        if (GCAA_Assignments(n).path(m) > -1)
            taskCurr = lookupTask(tasks, GCAA_Assignments(n).path(m));
            plot([GCAA_Assignments(n).times(m) GCAA_Assignments(n).times(m)+taskCurr.duration],[1 1],'-','color',Cmap(agents(n).type,:), 'Linewidth',10)

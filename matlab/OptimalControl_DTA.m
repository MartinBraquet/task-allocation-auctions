...
clear J J_to_completion_target X_full_simu p_GCAA_full_simu S_GCAA_ALL X;
...
for i_round = 1:n_rounds
    ...
    if use_GCAA
        % GCAA solution
        tic; [S_GCAA, p_GCAA, S_GCAA_ALL, rt_curr, Agents] = GCAASolution(Agents, G, Tasks)
    ...
    [X, completed_tasks_round, J_curr, J_to_completion_target(i_round+1,:)] = OptimalControlSolution(pos_a_loop, v_a_loop, pos_t, v_t, radius_t, p_GCAA, Agents, tf_t_loop, tloiter_t, time_step, n_rounds_loop, na, kdrag);
    ...
    for j = completed_tasks_round
            ...
            %pos_t(j,:) = [1 1] * 1e16;
            %U_completed_tot = U_completed_tot + S_GCAA_ALL(j);
    end
...
save(sprintf('mat/%s/simu_%d/p_GCAA_just_saved_CommLimit_%d.mat', simu_name, simu_number, CommLimit), 'p_GCAA_full_simu');
save(sprintf('mat/%s/simu_%d/S_GCAA_ALL_just_saved_CommLimit_%d.mat', simu_name, simu_number, CommLimit), 'S_GCAA_ALL_full_simu');

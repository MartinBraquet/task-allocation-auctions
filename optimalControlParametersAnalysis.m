    ...
    function [] = analyzeUtilityRewardSuccessProbability()
        ...
        for k = 2:length(nomReward)
            for l = 2:length(nomProbAT)
                ...
                totalUtilityAllocation(k,l) = S_CBBA;
...
%% Other subfunctions
    ...
    function [] = setPlotAllocation(pCBBA)
        
        % Find the optimal control solution for the given allocation p_CBBA
        [X, ~, ~, ~] = OptimalControlSolution(posA, vA, posT, vT, radiusT, pCBBA, Agents, tfT, Tasks.tloiter, timeStep, nRounds, nA, kdrag);
        plotMapAllocation(X, nRounds, nA, colors, 'CBBA solution');
